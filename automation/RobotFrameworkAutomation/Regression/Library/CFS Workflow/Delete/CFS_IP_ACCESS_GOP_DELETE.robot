...
*** Keywords ***
CFS IP Access GOP DELETE Design and Assign Validation
    ...
    ${CFS_IP_ACCESS_GOP_FTTH}    Get Service Item    ${order}    CFS_IP_ACCESS_GOP_FTTH
    ${objectId}    Get Service Item Characteristic    ${CFS_IP_ACCESS_GOP_FTTH}    eaiObjectId
...
CFS IP Access GOP DELETE Fetch Validation
...
CFS IP Access GOP DELETE Disconnect Line WBA Validation
...
CFS IP Access GOP DELETE NR Radius Configuration Validation
    [Arguments]    ${item}    ${taiName}
...
CFS IP Access GOP DELETE NR CGW Configuration Validation
    [Arguments]    ${item}    ${taiName}
    [Documentation]    Validates ROM Create Order towards NR
    ${processId}    Get TAI Process Id    ${taiName}    ${item}[id]
    ${request}    Get Outbound Payload    ${processId}    &{API_OPERATION}[NR_CREATE_ORDER]
...
CFS IP Access GOP DELETE Complete Project Validation
    [Arguments]    ${item}    ${taiName}
    ${processId}    Get TAI Process Id    ${taiName}    ${item}[id]
...
CFS IP Access GOP DELETE SR Validation
