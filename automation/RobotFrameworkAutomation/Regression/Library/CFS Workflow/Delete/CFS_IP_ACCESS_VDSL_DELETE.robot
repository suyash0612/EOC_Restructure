...
*** Keywords ***
CFS IP Access VDSL DELETE Design and Assign Validation
    ...
    ${objectId}    Get Service Item Characteristic    ${CFS_IP_ACCESS_WBA_VDSL}    eaiObjectId
...
CFS IP Access VDSL DELETE Fetch Validation
...
CFS IP Access VDSL DELETE Disconnect Line WBA Validation
...
CFS IP Access VDSL DELETE NR Radius Configuration Validation
    [Arguments]    ${item}    ${taiName}
...
CFS IP Access VDSL DELETE NR CGW Configuration Validation
    [Arguments]    ${item}    ${taiName}
    [Documentation]    Validates ROM Create Order towards NR
    ${processId}    Get TAI Process Id    ${taiName}    ${item}[id]
    ${request}    Get Outbound Payload    ${processId}    &{API_OPERATION}[NR_CREATE_ORDER]
...
CFS IP Access VDSL DELETE Complete Project Validation
    [Arguments]    ${item}    ${taiName}
    ${processId}    Get TAI Process Id    ${taiName}    ${item}[id]
...
CFS IP Access VDSL DELETE SR Validation
...
CFS IP Access VDSL DELETE NR DHCP Configuration Validation
    [Arguments]    ${item}    ${taiName}
    [Documentation]    Validates ROM Create Order towards NR
    ${processId}    Get TAI Process Id    ${taiName}    ${item}[id]
    ${request}    Get Outbound Payload    ${processId}    &{API_OPERATION}[NR_CREATE_ORDER]
    ${request_json}    evaluate    json.loads('''${request}''')    json
    #Validate Payload
    #Validate DHCP NTU Configuration
    #Validate IP Address
