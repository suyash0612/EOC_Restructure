...
*** Keywords ***
CFS IP Access NTU ADD Design and Assign Validation
    ...
    XML Validate Element by Value    ${request_xml}    //provisioningOrderItem/serviceType/text()    ipaccess:NtuEquipment
    # Validate Action
...
CFS IP Access NTU ADD Fetch Validation
...
CFS IP Access NTU ADD Complete Project Validation
    [Arguments]    ${item}    ${taiName}
    ${processId}    Get TAI Process Id    ${taiName}    ${item}[id]
...
CFS IP Access NTU ADD SR Validation
...
CFS IP Access NTU ADD Cancel Project Validation
    [Arguments]    ${item}    ${taiName}
    ${processId}    Get TAI Process Id    ${taiName}    ${item}[id]
